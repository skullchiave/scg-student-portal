...
## ヨリソルからの設問移行（2026-09-05）

ヨリソルの契約は **2027年2月で終了**し、来年度から小テストはこのポータルで行う。
設問は契約が生きているうちに CSV で書き出して持ってくる（切れたら二度と取り出せない）。
...
- ヨリソル側の形（9/5 に担当から入手した10問で確認）: 単一選択・**2択・正解1つ**＝こちらの
  `questions(prompt, choice_a, choice_b)` ＋ `question_answers` と**1対1で対応する**。作り直しは要らない
- 🔴 **設問文は HTML**（`<br />` と `<span style="color:#e74c3c;">`＝空欄の赤字）。
  `prompt` は `textContent` で描くので、タグを入れると文字として出てしまう。
  変換で `<br>`→改行・他タグは外す（**赤い色は失われる**。戻したいならポータル側に安全な描画を作る話になる）
- 改行を活かすため `.prompt` に `white-space:pre-line` を入れた（ヨリソルの設問は「A：…／B：…」の会話が改行で分かれる）
- 取り込みは必ず **`is_open=false`** で作る（中身を見てから教師画面で公開する）
- ⚠ SQL の正解列名は `correct` を既定にしている。ライブのスキーマと違ったら `--answer-col` で変える
- ⚠ 手元の10問は**全量ではない可能性が高い**（カテゴリがパス形式＝他の枝がある／ファイル名が「インポートデータ」）。
  担当への確認事項は Drive `0.2 claude-work\【学生ポータル】 Student Portal\ヨリソル_担当への確認事項.html`
