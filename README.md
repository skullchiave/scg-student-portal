...
🔴 **このリポは public なので、パスワードはここには書かない**（2026-09-12・教材1,686問を入れた日に決定）。
番号（ID）だけは公開情報。パスワードは `.env` を作って読む（下の「検証スクリプト」の前に3行で説明）。

| 役割 | ID | パスワード |
|---|---|---|
| 学生（デモ用） | s001 〜 s040 | `.env` の `SP_STUDENT_PW`（デモ学生は全員共通） |
| 教師 | t001 | `.env` の `SP_TEACHER_PW` |
| 負荷試験用学生 | l001 〜 l150 | `.env` の `SP_STUDENT_PW`（学生と共通） |

### `.env` を作る（初回だけ・3行）

1. `.env.example` を `.env` にコピーする（`copy .env.example .env`）
2. 右側の4つのキーに、ドライブの控え（`【学生ポータル】 Student Portal\デモのログイン情報（2026-09-11更新）.txt`）から値を書き込む
3. 保存するだけ。`.env` は `.gitignore` 済みなのでコミットされない（テスト・スクリプトは `tests/env_creds.py` 経由で自動的に読む）

★ `.env` が無いままテストを回すと、DBに触る部分は**エラーではなく skip** になる（`.env が無いので飛ばしました` と出る）。
...
- 入力: なし（対象URL・番号はスクリプト内に定義。パスワードだけ `.env` から読む）
- 出力: 各項目の PASS/FAIL、負荷テストは成功率と応答時間（平均/p95/最悪）
- 失敗時の見方: `test_security.py` の ❌ 行がそのまま「破られた防御」。負荷テストの NG例 行に
  エラー内容の先頭80字が出る。`.env` が無いとDBに触る部分は skip になる（上の「デモアカウント」参照）
