...
コードとSQLの構造だけを見る部分は DB不要・常に実行。
デモのDBに対して、保存・読み戻し・二重送信・順序・他人の下書きを実際に試す部分は
...
"""
import sys, io, os, re, json, unittest
...
class TestDraftsStructure(unittest.TestCase):
    """SQL・画面側の作り・文言の構造検査（DB不要・常に実行）"""
    ...
    def test_01_sql(self):
        ...
        self.check("★学生が下書きテーブルへ直接 書けない（insert/update/delete のポリシーを作っていない）",
              not re.search(r"for\s+(insert|update|delete)", low))
        self.check("自分の下書きだけ読める", "student_id = auth.uid()" in low)
        self.check("教師は読める（授業中に進み具合を見るため）", "is_teacher()" in low)
        self.check("save_draft は security definer", re.search(r"function public\.save_draft.*?security definer", low, re.S) is not None)
        self.check("save_draft の search_path が固定（乗っ取り対策）",
              re.search(r"function public\.save_draft.*?set search_path\s*=", low, re.S) is not None)
        self.check("別の回の設問を混ぜて保存できない", "does not belong to this quiz set" in sql)
        self.check("公開されていない回には保存させない", "is not open" in sql)
        self.check("提出後に下書きを捨てる関数がある", "function public.discard_drafts" in low)
        self.check("実行権限は authenticated だけ（anon には配らない）",
              low.count("grant execute") >= 2 and "to anon" not in low)
        self.check("★anon から明示的に取り上げている（revoke ... from public だけでは付き直す）",
              low.count("from anon") >= 2)

    def test_02_screen(self):
        print("\n=== 2. 画面側の作り（DB不要）===")
        idx = read(INDEX)
        self.check("下書きの関数がそろっている",
        ...
        self.check("★端末へ書くのが先、送信はあと（通信が死んでいても消えない）",
              rec.find("draftWriteLocal") != -1 and rec.find("draftWriteLocal") < rec.find("draftFlush"))
        self.check("答えを選ぶと下書きに入る（選択肢ボタンから draftRecord を呼ぶ）",
              re.search(r"b\.onclick\s*=\s*\(\)=>\{\s*draftRecord\(q\.id,\s*idx\)", idx) is not None)
        self.check("★保存するのは元の番号で、画面の並び順ではない",
              "// ★元の番号。画面の位置ではない" in idx and "data-idx" in idx)
        self.check("下書きの鍵に学生の id が入る（共用端末で他人の分を拾わない）",
              re.search(r"function draftKey[\s\S]{0,240}profile[\s\S]{0,80}id", idx) is not None)
        self.check("ログアウトで下書きを消す", "draftClearAll(); api.logout()" in idx)
        self.check("提出できたら下書きを捨てる", "discard_drafts" in idx and "draftClearLocal(quizSet.id)" in idx)
        self.check("提出の前に未送信ぶんを送り切る",
              re.search(r"await draftFlush\(\);[\s\S]{0,200}submit_attempt", idx) is not None)
        self.check("通信が戻ったら自動で送る", 'addEventListener("online"' in idx and "visibilitychange" in idx)
        self.check("続きから再開する（サーバーの下書きを読む）", "/rest/v1/attempt_drafts?select=" in idx)
        self.check("下書きが読めなくても受験は始められる（読み込み失敗を握りつぶす）",
              re.search(r"attempt_drafts[\s\S]{0,200}catch\(e\)\{\s*srv\s*=\s*\[\]", idx) is not None)
        self.check("未回答の最初の問題へ戻る", "firstBlank" in idx)
        self.check("保存の表示が提出と別の言葉になっている（q.sv.* を使う）", "q.sv.saved" in idx and "q.sv.pend" in idx)

    def test_03_wording(self):
        print("\n=== 3. 文言（DB不要）===")
        i18 = read(I18N)
        for k in ("q.sv.saving", "q.sv.saved", "q.sv.pend", "q.sv.resumed"):
            self.check(f"{k} が日本語と英語の両方にある", i18.count('"' + k + '"') >= 2)
        self.check("学生向けの文言がやさしい日本語（漢字だけの塊にしない）", "ほぞん" in i18)


@unittest.skipUnless(LIVE, "既定ではDB往復をskip。有効化するには環境変数 SP_LIVE=1 か --live 引数を付ける（先に db/2026-09-06_attempt_drafts.sql を流すこと）")
class TestDraftsLive(unittest.TestCase):
    """本物のDBで保存・読み戻し・二重送信・順序・他人の下書きを確認する"""
    ...
    def test_01_roundtrip(self):
        ...
        self.check("設問が2問以上取れる", len(qids) >= 2)

        if not (qs and len(qids) >= 2):
            self.skipTest("公開中の回・設問が無いため往復テストを飛ばす")
            return
        ...
        self.check("★遅れて届いた古い回答で新しい回答が消えない",
        ...
        self.check("★他の学生の下書きは見えない", st == 200 and (seen or []) == [], str(seen))

        # テーブルへ直接書けない
        st, _ = req("/rest/v1/attempt_drafts", tokA,
                    {"quiz_set_id": qs, "question_id": qids[1], "chosen": 1})
        self.check("★学生はテーブルへ直接 insert できない", st in (401, 403), str(st))
        ...
        if other:
            ...
            self.check("★別の回の設問は保存できない", st >= 400, str(st))
        else:
        ...
        st, _ = req("/rest/v1/rpc/save_draft", None,
                    {"p_quiz_set_id": qs, "p_question_id": qids[0], "p_chosen": 1, "p_client_seq": 1})
        self.check("★未ログイン(anon)では保存できない", st in (401, 403, 404), str(st))
        st, _ = req("/rest/v1/rpc/discard_drafts", None, {"p_quiz_set_id": qs})
        self.check("★未ログイン(anon)では下書きを消せない", st in (401, 403, 404), str(st))

        # 片付け
        st, n = req("/rest/v1/rpc/discard_drafts", tokA, {"p_quiz_set_id": qs})
        self.check("提出後の片付けで下書きが消える", st == 200 and isinstance(n, int) and n >= 1, f"{st} {n}")
        st, back = req(f"/rest/v1/attempt_drafts?select=question_id&quiz_set_id=eq.{qs}", tokA)
        self.check("消えたことを読み戻しで確かめた", (back or []) == [], str(back))
