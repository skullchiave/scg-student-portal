...
}
{
  ...
  {
    const stu = [
      { student_no: "2604999", name: "太郎", class_name: "A" },
      { student_no: "2604998", name: "花子", class_name: "A" },
      { student_no: "2604997", name: "次郎", class_name: "B" },
    ];
    const st2 = [{ id: "t1", title: "1-①" }, { id: "t2", title: "1-②" }];
    const bp = {
      "2604999t1": { score: 8, total: 10 },
      "2604999t2": { score: 6, total: 10 },
      "2604998t1": { score: 4, total: 10 },
      "2604997t1": { score: 10, total: 10 },
    };
    R.avg_rows = FmtImport.wideRows(stu, st2, bp);
  }
...
class NodeParityTest(CheckMixin, unittest.TestCase):
    ...
    def test_10c_results(self):
        ...
        self.check("右端に「受けた数・合計点・合計満点・平均点・達成率」がある",
                   wide[0][-5:] == ["受けた数", "合計点", "合計満点", "平均点", "達成率(%)"],
                   str(wide[0][-5:]))
        # 学生2人 + 空行1 + クラス平均1（Aのみ）= 4行。★クラスが1つなら全体平均は出さない
        self.check("1行＝1学生（＋下に平均の行）", len(wide) - 1 == 4, str(len(wide) - 1))
        ...
        self.check("受けた数・合計が合っている", [a[-5], a[-4], a[-3]] == [2, 13, 20],
                   str([a[-5], a[-4], a[-3]]))
        self.check("1件も受けていない列でも合計は崩れない", [b[-5], b[-4], b[-3]] == [1, 0, 10],
                   str([b[-5], b[-4], b[-3]]))
        self.check("🔴★0点の人の達成率は 0（空欄にしない）", b[-1] == 0, repr(b[-1]))
        ...
        ar = r["avg_rows"]
        head2 = ar[0]
        self.check("見出しの右端が 受けた数/合計点/合計満点/平均点/達成率(%)",
                   head2[-5:] == ["受けた数", "合計点", "合計満点", "平均点", "達成率(%)"], str(head2[-5:]))
        # 学生3人 + 空行1 + クラス平均2 + 全体平均1 = 7行（見出し除く）
        self.check("学生の行のあとに 空行・クラス平均・全体平均が付く", len(ar) - 1 == 7, str(len(ar) - 1))
        self.check("★平均行の前に空行が入る（並べ替えで学生と混ざらない）",
                   all(c == "" for c in ar[4]), str(ar[4]))
        self.check("★平均行は学籍番号が空（学生の行と見分けがつく）",
        ...
        # 全体: 1-① は 8,4,10 → 22/3 = 7.3
        self.check("全体平均も受けた人だけで割る", ar[7][3] == 7.3, str(ar[7][3]))
        # 個人の平均点: 太郎 (8+6)/2 = 7.0
        self.check("★個人の平均点＝合計点 ÷ 受けた数", ar[1][-2] == 7, str(ar[1][-2]))
        self.check("★達成率＝合計点 ÷ 合計満点（受けた分だけ）", ar[1][-1] == 70, str(ar[1][-1]))
